---
tags:
  - campaign/shadows-over-rome
  - npc
  - act/1
  - npc/secondary
  - source/npc-secondari-csv
  - faction/merchant-world
Faction: "Mercanti indipendenti"
race: "Halfling"
role: "Mercante di alimenti e materiali da cucina"
source_csv: "NPC_secondari.csv"
source: "Wazard/Shadows-over-Rome@gpt-main/CSVs/NPCs/NPC_secondari.csv"
---
# Vibius Panciotto Ennio

## Dati NPC

| Campo | Dettaglio |
| --- | --- |
| Fonte | `NPC_secondari.csv` |
| Fazione | Mercanti indipendenti |
| Stirpe | Halfling |
| Ruolo | Mercante di alimenti e materiali da cucina |

## Descrizione Estesa

Vibius Panciotto Ennio e una figura mercantile urbana legata a **Mercanti indipendenti**. Il tratto di partenza piu evidente e questo: Halfling largo e robusto, mani da cuoco e mercante, grembiule macchiato, volto stanco ma occhi svegli.

In scena dovrebbe avere una silhouette riconoscibile anche prima di parlare. Usa il modo in cui occupa lo spazio, guarda gli altri, tiene le mani o protegge i propri strumenti per far capire ai giocatori che non e una comparsa generica. Anche quando compare per pochi minuti, Vibius Panciotto Ennio dovrebbe lasciare un dettaglio facile da ricordare: un odore, una postura, un tic, un oggetto consumato, una frase ripetuta o una reazione sproporzionata a qualcosa di piccolo.

Durante il Breaching Festival e la Remembrance Moon, la sua presenza va colorata con la citta in movimento: folla, candele, merci in ritardo, musica, fame trattenuta, lutto pubblico e prezzi che cambiano. Non serve che conosca il grande complotto; basta che mostri come il complotto tocca la vita quotidiana.

## Personalita

Pratico, preoccupato, diretto. Non è eroico ma vuole risposte.

Al tavolo, interpreta Vibius Panciotto Ennio con un obiettivo semplice e immediato. Vuole conservare qualcosa: reputazione, sicurezza, profitto, fede, lavoro, segreti o dignita. Se i PG danno a questa persona modo di ottenere quel bisogno senza umiliarlo, diventa piu utile. Se lo mettono all'angolo, reagisce in modo pratico prima che drammatico: mente, prende tempo, chiama qualcuno, chiude una porta o cambia versione.

## Uso in Campagna

Introduce la falsa pista di Brutus Verro tramite carichi non registrati.

Funzione consigliata: rendere concreti prezzi, scorte, trattative e paura della scarsita. Parla sempre come se ogni minuto avesse un costo.

Vibius Panciotto Ennio non deve spiegare la trama al posto dei PG. Deve dare attrito, colore e conseguenze. Quando serve un indizio, fallo arrivare attraverso qualcosa che ha visto, comprato, registrato, sentito o temuto, non attraverso una confessione completa.

## Impressione per i Giocatori

Sembra un mercante onesto e arrabbiato per i ritardi.

La prima impressione deve essere giocabile in una frase: i PG dovrebbero capire subito se questa persona sembra utile, pericolosa, fragile, irritante, rispettabile o compromessa. La verita puo essere piu complessa, ma il primo impatto deve aiutare il tavolo a scegliere come avvicinarla.

## Spunti di Scena

- Usa Vibius Panciotto Ennio quando servono prezzi, merci mancanti, pettegolezzi di mercato o una trattativa che riveli chi sta soffrendo la scarsita.
- Se i PG trattano Vibius Panciotto Ennio con rispetto, puo offrire una voce, una scorciatoia sociale o un dettaglio osservato per caso.
- Se i PG fanno pressione nel modo sbagliato, Vibius Panciotto Ennio si chiude, chiede protezione a qualcuno o trasforma una scena semplice in reputazione negativa.

## Segreti, Pressione e Reazioni

- **Se aiutato:** Vibius Panciotto Ennio ricorda il favore e puo restituirlo con informazioni minori, accesso sociale, una voce o un avvertimento.
- **Se minacciato:** prova a proteggersi prima di dire la verita completa; se appartiene a una rete, avverte qualcuno piu in alto.
- **Se ignorato:** continua la propria routine e puo riapparire piu avanti come testimone, cliente, ostacolo o vittima indiretta degli eventi.
- **Sotto pressione del festival:** diventa piu frettoloso, piu emotivo o piu disposto a piccoli compromessi, perche la citta intera sta chiedendo troppo a tutti.

## Battute Rapide

- "Non ho detto di sapere tutto. Ho detto di sapere abbastanza da voler restare fuori dai guai."
- "Oggi Roma ha troppa musica per sentire bene le bugie."
- "Se volete una risposta pulita, chiedete a qualcuno che non rischia nulla."
